...
import { AGENCY_CONFIG, getSystemPrompt, type Agency } from './constants/agency';
import { announce } from './services/elevenLabsService';
...
export default function App() {
  ...
  const [agency, setAgency] = useState<Agency>('EMS');
  const config = AGENCY_CONFIG[agency];

  const handleAgencyChange = (newAgency: Agency) => {
    setAgency(newAgency);
    announce.agencySelected(newAgency).catch(() => {});
  };
...
- Interrupt immediately with a clear verbal "SAFETY ALERT" and specific instructions if a hazard is detected.`;
  ...
  const { status, connect, disconnect, sendAudio, sendVideoFrame, transcript: fieldTranscript } = useGeminiLive(
    dutyStatus === 'TRAINING' ? trainingInstruction : getSystemPrompt(agency),
  ...
  return (
    <div className={cn(
      "min-h-screen text-cad-text font-mono flex flex-col selection:bg-cad-green selection:text-black transition-all duration-500",
        ...
        <nav className={cn(
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              <Shield className={cn("w-4 h-4", dutyStatus === 'TRAINING' ? "text-cad-amber" : config.accent)} />
              <span className="text-xs font-bold tracking-tighter uppercase">
                {dutyStatus === 'TRAINING' ? 'TRAINING SIM // UNIT 01-A' : `${agency} CAD // UNIT 01-A`}
              </span>
            </div>
            <div className="flex items-center gap-1 ml-2">
              {(['EMS', 'FIRE', 'POLICE'] as Agency[]).map((a) => (
                <button
                  key={a}
                  onClick={() => handleAgencyChange(a)}
                  className={cn(
                    "px-2 py-0.5 text-[9px] font-bold uppercase tracking-widest border transition-all",
                    agency === a
                      ? AGENCY_CONFIG[a].buttonActive
                      : "bg-black/40 text-cad-muted border-cad-border hover:text-white"
                  )}
                >
                  {a}
                </button>
